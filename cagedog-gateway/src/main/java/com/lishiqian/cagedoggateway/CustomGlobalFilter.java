...
import com.lishiqian.cagedogclientsdk.utils.SignUtils;
...
//    private static final List<String> IP_WHITE_LIST = Arrays.asList("127.0.0.1");
    ...
    public static final String INTERFACE_HOST = "http://139.159.207.177:8123";
...
//        path = INTERFACE_HOST + request.getPath().value();
        // 5.该用户对该接口是否还有调用次数
        UserInterfaceInfo infoServiceById = null;
        try {
            infoServiceById = innerUserInterfaceInfoService.getById(interfaceInfo.getId(), invokeUser.getId());
        }catch (Exception e){
            log.error("getInterfaceInfoById error",e);
        }
         if(infoServiceById.getLeftNum()<0){
             log.error("次数不够 error");
         }
